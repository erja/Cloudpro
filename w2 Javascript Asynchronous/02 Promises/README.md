...
We will employ a pattern that uses future-facing objects called **Promises**, designed to deal with things that happen at any time in the future. See  [Using Promises](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises) or the [HTML5Rocks blog post](http://www.html5rocks.com/en/tutorials/es6/promises/) in the resources for further discussion of the type of code you will be working with in this lab.
...
- Begin with a quick review of the *Core Worksheet* which contains analysis of `ajax_sync.html`. That file contains code which synchronously gets URLs and populates the DOM with the contents parsed from the returned JSON data.
...
- Note the final block of code executed here:
  ...
  - The `map(getJson)` uses map function to map each chapter URL to a “promise to supply some JSON” 
...
The `map` method is a Javascript method that creates a new typed array with the results of calling a provided function on every element in this array, for example arr.map(function(x) {return x*2;}) 
creates a new array where every element is multiplied by 2. 
```
var array1 = [1, 4, 9, 16];
...
Try to define a map-reduce chain of promises corresponding to button click events. Map and reduce are functional-style operations on a list or an array. This exercise is a little beyond this course but if you want a peek on functional programming, familiarize yourself with these concepts.
